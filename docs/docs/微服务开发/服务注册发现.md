...
sidebar_position: 5
hide_title: true
keywords: [GoFrame,GoFrame框架,服务注册发现,gsvc组件,etcd,zookeeper,polaris,HTTP,GRPC,SEO优化]
description: '本文档详细介绍了GoFrame框架中服务注册发现组件的使用和实现，主要由gsvc组件管理，并支持etcd、zookeeper、polaris等多种实现方式。通过合理配置，全局启用服务注册发现以提高服务调用的灵活性和可扩展性，帮助开发者了解如何在服务请求中应用或关闭服务发现特性，是构建微服务架构的必备指南。'
---
