...
sidebar_position: 1
hide_title: true
keywords: [代理部署,GoFrame,反向代理,Nginx,静态文件,Golang应用,WebServer,动态请求,负载均衡,域名配置]
description: '本文档介绍如何使用Nginx作为反向代理的前端接入层来部署GoFrame框架应用。通过配置静态文件后缀或目录，可以有效分离静态和动态请求，以提高性能。配置示例详细展示了如何将请求转发到Golang应用，实现专业性的WebServer部署方案。'
---
