...
sidebar_position: 0
hide_title: true
keywords: [GoFrame,GoFrame框架,中间件,拦截器,HTTP HandlerFunc,CORS,请求拦截,全局中间件,分组路由,执行优先级]
description: '本章介绍了GoFrame框架中中间件和拦截器的基本使用。GoFrame通过优雅的中间件请求控制方式和经典的中间件洋葱模型，为WebServer提供灵活强大的插件机制。讨论了中间件定义、类型、注册方式及执行优先级，以提供全面的请求流程控制方案。'
---
