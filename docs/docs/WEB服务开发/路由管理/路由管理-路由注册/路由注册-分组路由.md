...
sidebar_position: 2
hide_title: true
keywords: [GoFrame,GoFrame框架,分组路由,路由注册,路由优化,HTTP Method,中间件,批量注册,层级注册,RESTful风格]
description: '本页介绍了如何在GoFrame框架中使用分组路由进行路由注册。分组路由是一种推荐的路由注册方式，支持层级和批量注册。用户可以为每个路由设置独特的HTTP Method以及应用中间件。通过这些特性，开发者能更高效地管理和优化项目的路由结构。'
---
