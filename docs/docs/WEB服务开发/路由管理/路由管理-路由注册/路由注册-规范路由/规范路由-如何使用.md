...
sidebar_position: 1
hide_title: true
keywords: [GoFrame,GoFrame框架,路由方法定义,请求对象,响应对象,规范化路由,Bind方法,BindHandler,数据校验,OpenAPIv3]
description: '本文档介绍了如何在GoFrame框架下定义路由方法、管理和注册路由，以及请求对象和返回对象的规范化结构。通过使用分组路由的Bind方法和基本的BindHandler方法，我们可以有效地管理路由。还包括如何进行输入数据校验以及使用统一的后置中间件进行数据处理。'
---
