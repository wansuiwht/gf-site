...
description: '本文介绍了如何在GoFrame框架中使用gmetric组件进行监控指标的开发。通过引入otelmetric组件，可以利用OpenTelemetry实现框架的监控指标接口。文章详细描述了指标管理对象的创建、各种监控指标对象的使用及初始化方法，并通过代码示例展示了如何读取和操作指标数据。'
---
