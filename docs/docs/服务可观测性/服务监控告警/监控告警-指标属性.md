...
description: '本文介绍了如何在GoFrame框架中的监控告警组件中使用指标属性进行过滤、汇总和统计。提供了常量属性、变量属性和全局属性三种属性注入方式，并通过具体示例演示了如何在不同场景下应用这些属性。结合OpenTelemetry和Prometheus，展示了如何定义和应用指标属性，以实现灵活高效的数据监控和分析。'
---
