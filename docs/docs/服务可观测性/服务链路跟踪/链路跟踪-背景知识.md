...
description: '本文介绍了OpenTelemetry项目的背景和重要概念，包括TracerProvider、Tracer、Span、Attributes、Events、SpanContext和Propagator等组件，并说明了GoFrame框架在这些技术上的支持，以及如何使用gtrace模块实现链路跟踪。此外，还列举了支持OpenTelemetry标准的GoFrame核心组件，如HTTP客户端、HTTP服务端、gRPC客户端和服务端、Logging、ORM和NoSQL Redis等。'
---
