...
description: '这篇文档展示了如何在单进程中使用GoFrame进行链路跟踪，通过创建Root Span来追踪方法调用链，并在Jaeger UI中查看结果。介绍了如何在方法间创建Span以记录方法调用过程，并通过详细的示例代码展示了如何在GoFrame框架中实现链路的信息传递与性能监控，帮助用户快速定位系统异常和发现性能瓶颈。'
---
