---
slug: '/core/gres/pack-using-cli'
title: '资源管理-工具打包'
sidebar_position: 0
hide_title: true
---

我们可以通过 `gf` 命令行工具的 `pack` 命令实现对任意文件/目录的打包，关于 `gf` 命令行工具的安装和使用具体请查看 [资源打包-pack](../../3-开发工具/7-资源打包-pack.md) 章节。由于通过命令行工具进行打包比较简便，因此也是推荐的打包方式。
