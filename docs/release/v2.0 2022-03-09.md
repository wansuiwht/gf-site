...
- `Entity/DAO/DO` 特性
- 面向接口化设计
- 更多详细介绍： [工程开发设计(🔥重点🔥)](../docs/框架设计/工程开发设计.md)
...
- 自动生成 `SwaggerUI` 页面
- 更多详细介绍： [路由注册-规范路由](../docs/WEB服务开发/路由管理/路由管理-路由注册/路由注册-规范路由.md)
...
- 详细介绍： [数据库ORM🔥](../docs/核心组件/数据库ORM.md)
...
1. `/net/ghttp`
1. 新增路由注册方式： [路由注册-规范路由](../docs/WEB服务开发/路由管理/路由管理-路由注册/路由注册-规范路由.md)
2. 默认将 `Request` 对象注入到 `ctx` 上下文对象中，并增加 `RequestFromCtx/g.RequestFromCtx` 方法获取 `ctx` 中的 `Request` 对象。
3. 将 `Client` 功能特性进行抽离，封装为 `gclient` 组件： [HTTPClient](../docs/WEB服务开发/HTTPClient.md)
...
2. 完善全新的链路跟踪使用文档： [服务链路跟踪](../docs/服务可观测性/服务链路跟踪.md)
...
05. `/os/gcfg`
    1. 采用适配器模式，以接口化设计重构该组件，以提高扩展性： [配置管理-接口化设计](../docs/核心组件/配置管理/配置管理-接口化设计.md)
    2. 默认提供了基于文件系统的配置管理实现： [配置管理](../docs/核心组件/配置管理.md)
    ...
    8. 配置组件易用性改进，通过单例对象访问配置组件将会按照 `toml/yaml/yml/json/ini/xml` 文件后缀自动检索配置文件： [配置管理](../docs/核心组件/配置管理.md)
06. `/os/gcmd`
...
12. `/os/gsession`
    1. `gsession.Storage` 接口增加 `ctx` 上下文参数输输入，用于承接上下文信息、实现完整的链路跟踪。并未保证严谨性增加 `error` 返回参数： [Session](../docs/WEB服务开发/Session.md)
