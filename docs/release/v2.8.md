...
1. 组件`container/gring`标记废弃，未来不再继续维护。
2. 组件`contrib/trace/jaeger`已从源码仓库删除。
3. 组件`encoding/gjson`的`Load*`方法参数，从`interface{}`调整为了`[]byte`参数，以提高性能。
4. 组件`net/ghttp`的`StartPProfServer`方法端口参数从`port int`调整为了`address string`以提高灵活性。
5. 组件`net/gudp`重构，个别方法参数有调整：https://

## 组件改进
1. `database/gdb`
   - 新增`Model.Exist`方法，用于判断满足所给条件的数据是否存在：https://
   - 新增`OrderRandomFunction`接口方法，并为各个数据库实现支持`OrderRandom`排序方法。
   - 改进`Model.Fields`方法，增加对`gdb.Raw`类型参数的支持：https://
   - 新增对数据库`time/year`字段类型的支持：https://
   - 改进`orm`标签，增加对`unscoped`的支持：https://
