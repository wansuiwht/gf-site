...
根据我的分析，你这个 `RequestID` 的格式和 `TraceID` 规范非常切合，只是中间多了 `-` 符号。建议在自己的 `Server` 内部第一层中间件中将 `RequestID` 转换为 `TraceID`，通过自定义 `TraceID` 的方式注入到 `Context` 中，并将该 `Context` 传递给后续业务逻辑。
