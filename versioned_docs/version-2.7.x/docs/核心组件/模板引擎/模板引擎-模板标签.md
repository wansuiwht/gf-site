...
description: 'GoFrame框架中的模板引擎与模板标签基本用法，包括如何自定义模板闭合标签，使用pipeline在模板中传递数据，条件语句if...else的使用，range用于迭代slice、map等类型，使用define定义模板块，template标签结合define实现模板嵌套和复用，以及include标签在GoFrame框架中的使用差异。'
---
