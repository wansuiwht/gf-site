...
默认情况下，日志文件名称以当前时间日期命名，格式为 `YYYY-MM-DD.log`，我们可以使用 `SetFile` 方法来设置文件名称的格式，并且文件名称格式支持 [gtime (时间管理模块)](output/goframe-v1.15-md/模块列表/系统相关/gtime%20-时间管理模块) 时间格式 。简单示例：
