...
3. 我们在程序启动时通过以下方法设置当前数据库缓存管理的适配器为 `redis`。关于缓存适配器的介绍感兴趣可以参考 [缓存管理-接口设计](output/goframe-v2.1-md/核心组件-重点/缓存管理/缓存管理-接口设计) 章节。
...
5. 在 `ORM` 的操作中，这里使用 `Cache` 方法缓存查询结果到 `redis` 中，并在删除操作中也使用 `Cache` 方法清除 `redis` 中的缓存结果。关于 `ORM` 的缓存管理介绍请参考 [ORM链式操作-查询缓存](output/goframe-v2.1-md/核心组件-重点/数据库ORM/ORM链式操作-重点/ORM链式操作-查询缓存) 章节。

## 效果查看

**启动服务端：**

![](/markdown/2fd9f2b66254111dd3b82cac470144ff.png)

**启动客户端：**

![](/markdown/6ddbb1aecf51ca2f38f0bcfd91ac4e39.png)

在 `Jaeger` 上查看链路信息：

![](/markdown/45ea55b4fb3daae1dae65d4bbda6854d.png)

可以看到，这次请求总共产生了 `14` 个 `span`，其中客户端有 `4` 个 `span`，服务端有 `10` 个 `span`，每一个 `span` 代表一个链路节点。不过，我们注意到，这里产生了 `3` 个 `errors`。我们点击详情查看什么原因呢。

![](/markdown/d4b85239dc3d23490e8f098760445f5a.png)

我们看到好像所有的 `redis` 操作都报错了，随便点击一个 `redis` 的相关 `span`，查看一下详情呢：

![](/markdown/d5b4eca418d1bcc4a740ec5924eaa270.png)
...
![](/markdown/33eded13e02b502cfc5b38cae5038821.png)

再把上面的客户端运行一下，查看 `jaeger`：

![](/markdown/ee40ebfa736f4d25262e3d96ae289b67.png)

![](/markdown/699c921bfebb33129c00c12f2c7f09ce.png)
...
![](/markdown/8426fddd2fefd6416bb2092db79f3a86.png)
...
![](/markdown/ea2bdec838bfa2c021f144b6c454277f.png)
...
![](/markdown/b4aa32592e76f61a85f0e2876ef74b5f.png)
...
![](/markdown/107fddd5fe7699360ccb15160df11fdb.png)
