...
将 `timestamp` 时间戳变量进行时间日期格式化，类似PHP的 `date` 方法， `format` 参数支持 [PHP date](http://php.net/manual/en/function.date.php) 方法格式，亦可参考 [时间管理-gtime](output/goframe-v2.1-md/组件列表/系统相关/时间管理-gtime) 。
