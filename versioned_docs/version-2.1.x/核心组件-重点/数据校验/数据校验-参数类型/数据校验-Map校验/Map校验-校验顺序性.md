...
可以看到，我们想要校验结果满足顺序性，只需要将 `rules` 参数的类型修改为 `[]string`，按照一定的规则设定即可，并且 `msgs` 参数既可以定义到 `rules` 参数中，也可以分开传入（使用第三个参数）。 `rules` 的这种满足顺序性校验结果返回的规则，我们称之为 `gvalid tag`，与前面Struct校验章节介绍的 `gvalid tag` 一致，具体请参考章节 [Struct校验-基本使用](../数据校验-Struct校验/Struct校验-基本使用.md) 。