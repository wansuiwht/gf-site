...
2. `Value/Record/Result` 用于ORM操作的结果数据类型，具体说明请查看 [ORM结果处理](output/goframe-v1.14-md/核心组件/数据库ORM/ORM结果处理) 章节；
...
> 有这么多对象获取方式原因在于 `GF` 是一个模块化设计的框架，每个模块皆可单独使用。为了方便开发者使用常用的一些模块，因此框架也提供了一个 `g` 模块，这是一个高度耦合的模块，封装了一些常用对象的单例获取方式，详见 [对象管理](output/goframe-v1.14-md/核心组件/对象管理) 章节。
...
额外接入新的数据库相当方便，可参考源码中关于 `PostgreSQL`、 `SQLite`、 `Oracle`、 `SQL Server` 的接入方式。具体介绍请参考后续 [驱动开发](output/goframe-v1.14-md/核心组件/数据库ORM/ORM接口开发/驱动开发) 章节。