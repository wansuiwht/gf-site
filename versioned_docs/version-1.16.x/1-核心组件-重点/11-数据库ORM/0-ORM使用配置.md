...
各数据库类型更详细的 `link` 参数信息请查看对应引擎官网，参考 [数据库ORM](output/goframe-v1.16-md/核心组件-重点/数据库ORM) 章节。
...
其中 `database.logger` 即为 `gdb` 的日志配置，当该配置不存在时，将会使用日志组件的默认配置，具体请参考 [日志组件](output/goframe-v1.16-md/核心组件-重点/日志组件) 章节。
