...
`Request` 对象支持非常完美的请求校验能力，通过给结构体属性绑定 `v` 标签即可。由于底层校验功能通过 `gvalid` 模块实现，更详细的校验规则和介绍请参考 [数据校验-Struct校验](../../2-核心组件/6-数据校验/3-数据校验-参数类型/1-数据校验-Struct校验/1-数据校验-Struct校验.md) 章节。

以下示例代码中的请求参数转结构体方式适用于框架 `v1` 版本， `v2` 版本开始建议通过规范路由实现自动化的参数结构体转换和校验： [路由注册-规范路由](../1-路由管理/1-路由管理-路由注册/3-路由注册-规范路由/3-路由注册-规范路由.md)
...
可以看到，当错误产生后，我们可以通过 `err.(gvalid.Error)` 断言的方式判断错误是否为校验错误，如果是的话则返回第一条校验错误，而不是所有都返回。更详细的错误控制方法，请参考 [数据校验-校验结果](../../2-核心组件/6-数据校验/2-数据校验-校验结果.md) 章节。
