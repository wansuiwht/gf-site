...
| `sqlite` | `sqlite::@file(/var/data/db.sqlite3)` | [go-sqlite3](https://github.com/mattn/go-sqlite3) |
...
其中 `database.logger` 即为 `gdb` 的日志配置，当该配置不存在时，将会使用日志组件的默认配置，具体请参考 [日志组件-配置管理](output/goframe-v2.2-md/核心组件-重点/日志组件/日志组件-配置管理) 章节。
